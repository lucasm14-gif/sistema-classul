...
const logInsert = db.prepare(
  'INSERT INTO message_log (order_id, status_trigger, phone, body, success, error) VALUES (?, ?, ?, ?, ?, ?)'
);
const alreadySentStmt = db.prepare(
  'SELECT 1 FROM message_log WHERE order_id = ? AND status_trigger = ? AND success = 1 LIMIT 1'
);
...
export async function notifyStatus(order, status, { force = false } = {}) {
  ...
  if (!force && alreadySentStmt.get(order.id, status)) {
  ...
  try {
    ...
    logInsert.run(order.id, status, phone, body, 1, null);
    return { sent: true, phone, body };
  } catch (err) {
    logInsert.run(order.id, status, phone, body, 0, err.message);
