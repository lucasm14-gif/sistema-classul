...
import { analyzeChat } from './analyst.js';
...
async function saveMedia({ phone, chatName, messageId, kind, fallbackMime, fromMe }) {
  ...
  // Sentido do arquivo: o que vem do cliente é material para a arte; o que sai
  // daqui é a arte enviada para ele conferir.
  const category = fromMe ? 'arte' : 'material';
  ...
  if (order) {
    ...
    await q(
      ...
      [order.id, file.id, file.name, file.mimeType || mimeType, file.size ? Number(file.size) : buffer.length, file.webViewLink || null, category]
    );
    return { saved: 'pedido', order_number: formatOrderNumber(order.id), name: file.name, category };
  }

  // Arte nossa sem pedido aberto não vai para a caixa de entrada (encheria de
  // print). Só o material do cliente fica esperando um pedido.
  if (fromMe) return { skipped: 'arte sem pedido aberto' };
...
export async function handleWatchedMessage(body) {
  ...
  // Guarda os dois sentidos: material do cliente e a arte que você mandou
  // para ele conferir. Figurinha e áudio ficam de fora.
  let file = null;
  if (media && media.kind !== 'figurinha' && media.kind !== 'audio') {
    try {
  ...
  // Analisa depois de uma mensagem DO CLIENTE (é o que muda o rumo da conversa).
  // O próprio analista segura a frequência; um erro aqui não afeta o resto.
  let analysis = null;
  if (!fromMe) {
    try {
      analysis = await analyzeChat(phone);
    } catch (err) {
      console.error('watcher: falha ao analisar:', err.message);
      analysis = { error: err.message };
    }
  }

  return { watched: true, phone, media: media?.kind || null, file, analysis };
}
