...
// Baixa a mídia de uma mensagem recebida (foto, arte, documento) via Evolution.
// Pode falhar em alguns tipos de mensagem — quem chama trata o erro sem quebrar.
export async function fetchMediaBase64(messageId) {
  const { base, apikey, instance } = await evolutionConfig();
  ...
  if (!res.ok) {
    const body = await res.text().catch(() => '');
    throw new Error(`Evolution não devolveu a mídia (${res.status}): ${body.slice(0, 200)}`);
  }
  const data = await res.json();
  if (!data?.base64) throw new Error('Evolution respondeu sem o conteúdo do arquivo.');
  return {
    base64: data.base64,
