...
export async function createUploadSession(order, meta, origin) {
  return createUploadSessionInFolder(await ensureOrderFolder(order), meta, origin);
}

// Pasta única das fotos da extensão (dentro da pasta raiz da Classul no Drive).
export async function ensurePhotosFolder() {
  const s = await getSettings();
  if (s.google_photos_folder_id) return s.google_photos_folder_id;
  const rootId = await ensureRootFolder();
  const folder = await createFolder('Classul - Fotos da extensão', rootId);
  await setSettings({ google_photos_folder_id: folder.id });
  return folder.id;
}

// Baixa o conteúdo do arquivo (usado para servir a foto ao site/extensão).
export async function downloadFile(fileId) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Falha ao baixar do Drive (${res.status}): ${body.slice(0, 200)}`);
  }
  return Buffer.from(await res.arrayBuffer());
}

export async function createUploadSessionInFolder(folderId, { name, mimeType, size }, origin) {
  const token = await getAccessToken();
  const headers = {
...
// Pasta onde ficam os arquivos recebidos no WhatsApp que ainda não têm pedido.
export async function ensureInboxFolder() {
  const s = await getSettings();
  if (s.google_inbox_folder_id) return s.google_inbox_folder_id;
  const rootId = await ensureRootFolder();
  const folder = await createFolder('Classul - Recebidos do WhatsApp', rootId);
  await setSettings({ google_inbox_folder_id: folder.id });
  return folder.id;
}

// Upload feito pelo próprio servidor (o arquivo veio da Evolution, não do navegador).
export async function uploadBufferToFolder(folderId, { name, mimeType, buffer }) {
  const token = await getAccessToken();
  const boundary = `classul-${Date.now()}-${Math.random().toString(16).slice(2)}`;
  ...
  if (!res.ok) {
    ...
    throw new Error(`Falha ao subir o arquivo no Drive (${res.status}): ${text.slice(0, 200)}`);
  }
  return res.json();
}
