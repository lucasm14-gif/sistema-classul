...
export async function buildFinanceSummary(apiKey, items, { days = 30 } = {}) {
  ...
  for (const it of items) {
    ...
    try {
      ...
      for (const inv of invs) {
        ...
        investTotal += bal;
        investList.push({ type: inv.type || inv.subtype || 'OTHER', balance: bal });
      }
    } catch {
  ...
  // Contas bancárias agrupadas por instituição (com participação % no total).
  const bankMap = new Map();
  for (const a of accountsOut) {
    if (a.type === 'CREDIT') continue;
    const g = bankMap.get(a.connector) || { name: a.connector, image: a.connector_image, total: 0, count: 0 };
    g.total += a.balance;
    g.count += 1;
  ...
  // Cartões individuais (nome + final), do maior saldo devedor ao menor.
  const cards = accountsOut
  ...
  for (const iv of investList) {
    const cls = investClass(iv.type);
    const g = invMap.get(cls) || { name: cls, total: 0, count: 0 };
    g.total += iv.balance;
    g.count += 1;
    invMap.set(cls, g);
  }
  const investments = [...invMap.values()]
