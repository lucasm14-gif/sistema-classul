import pg from 'pg';
import crypto from 'crypto';
import { DEFAULT_QUICK_MESSAGES, DEFAULT_CATALOG } from './default-content.js';

export const STATUSES = ['novo', 'producao', 'pronto', 'entregue'];

const DEFAULT_SETTINGS_KEYS = [
  'evolution_url',
  'evolution_apikey',
  'evolution_instance',
  'msg_pronto_enabled',
  'msg_pronto_template',
  'msg_entregue_enabled',
  'msg_entregue_template',
  'google_client_id',
  'google_client_secret',
  'google_refresh_token',
  'google_folder_id',
  'google_photos_folder_id',
  'google_inbox_folder_id',
  'bot_enabled',
  'openai_api_key',
  'openai_model',
  'bot_test_number',
  'bot_products',
  'bot_system_prompt',
  'bot_webhook_secret',
  // Hermes: bot próprio rodando na VPS, que controla o sistema por ferramentas.
  'bot_engine',
  'hermes_enabled',
  'hermes_token',
  'hermes_webhook_url'
];

const DEFAULT_BOT_PRODUCTS =
  '- Placa de homenagem (tamanhos: 9x14, 12x17, 14x20, 16x25, 20x30 cm)\n' +
  '- Tarjeta plaqueta do Exército Brasileiro (EB) — plaqueta militar de identificação\n' +
  '- Placa de inauguração\n' +
  '- Placa em quadro (quadro de parede)\n' +
  '- Placa para jazigo\n' +
  '- Placa em aço inox escovado\n' +
  '- Troféus e estatuetas personalizadas\n' +
  '- Medalhas personalizadas\n' +
  '- Pins e botons\n' +
  '- Estojo luxo de veludo — vendido também SOZINHO, sem placa (cores: preto, azul e vermelho; tamanhos para placas de 9x14, 12x17, 14x20, 16x25 e 20x30 cm)';

const DEFAULT_BOT_PROMPT =
  'Você é o atendente virtual de PRÉ-ATENDIMENTO da Classul, empresa de placas e brindes personalizados de Porto Alegre-RS.\n\n' +
  'Seu papel é APENAS o primeiro contato: dar boas-vindas, entender o que o cliente precisa e coletar as informações iniciais. ' +
  'Você NÃO fecha pedido, NÃO passa preço, NÃO define prazo e NÃO cria arte — tudo isso é feito por um atendente humano da Classul logo depois. Deixe isso claro de forma gentil.\n\n' +
  'Fale como uma pessoa de verdade: cordial, simples e direto, em português do Brasil, com mensagens curtas (estilo WhatsApp). Pode usar no máximo 1 emoji por mensagem.\n\n' +
  'PRODUTOS QUE PRODUZIMOS (somente estes):\n{PRODUTOS}\n\n' +
  'Se o cliente pedir algo que NÃO está na lista acima, diga com educação que no momento não estamos produzindo esse item e ofereça os que fazemos.\n\n' +
  'Antes de encerrar, entenda o essencial:\n' +
  '- Qual produto da lista o cliente quer\n' +
  '- Uma ideia do que ele deseja (ocasião, texto/homenagem, tamanho quando fizer sentido, quantidade)\n\n' +
  'TAMANHOS: sempre que o cliente perguntar ou falar sobre tamanho, informe TODOS os tamanhos da placa de homenagem: 9x14, 12x17, 14x20, 16x25 e 20x30 cm. Nesse caso, envie também a foto [[FOTO:placa_homenagem]].\n\n' +
  'ESTOJOS AVULSOS: o estojo de veludo também é vendido SOZINHO, sem a placa — se o cliente quiser só o estojo, atenda normalmente. Cores: preto, azul e vermelho. ' +
  'ATENÇÃO ao tamanho: as medidas (9x14, 12x17, 14x20, 16x25 e 20x30 cm) se referem ao tamanho da PLACA QUE CABE DENTRO do estojo, e NÃO à medida externa dele. ' +
  'Diga sempre assim: "estojo para placa de 14x20", nunca "estojo de 14x20". Ao falar de estojos, envie [[FOTO:estojos]].\n\n' +
  'FOTOS: SEMPRE que o cliente perguntar sobre um produto, demonstrar interesse ou pedir para ver, envie as fotos daquele produto. Para enviar, escreva numa linha separada exatamente o marcador do produto (o sistema envia todas as fotos dele):\n' +
  '[[FOTO:placa_homenagem]] — placas de homenagem em estojo de veludo\n' +
  '[[FOTO:placa_inauguracao]] — placa de inauguração\n' +
  '[[FOTO:plaqueta_eb]] — plaquetas de identificação do Exército (EB)\n' +
  '[[FOTO:estojos]] — estojos de luxo em veludo (preto, azul, vermelho, vários tamanhos)\n' +
  'Envie o marcador do produto certo assim que o cliente disser qual produto quer. Não peça permissão para mandar as fotos, apenas mande.\n\n' +
  'Quando já tiver entendido o essencial, faça um breve resumo do que entendeu para o cliente, avise que um atendente da Classul vai dar sequência (arte, valores e prazo) e se despeça. ' +
  'Só nesse momento, escreva numa linha separada, exatamente ao final: [[ATENDIDO]]\n\n' +
  'Nunca invente preços, prazos ou informações. Não escreva [[ATENDIDO]] antes de entender o pedido.';

function defaultSettings() {
  return {
    evolution_url: process.env.EVOLUTION_URL || '',
    evolution_apikey: process.env.EVOLUTION_APIKEY || '',
    evolution_instance: process.env.EVOLUTION_INSTANCE || '',
    msg_pronto_enabled: '1',
    msg_pronto_template:
      'Olá {nome}! 🎉\n\nSeu pedido {pedido} está PRONTO!\n\n🔑 Código de retirada: {codigo}\nInforme esse código na hora de pegar o pedido.\n\nJá pode combinar a retirada ou aguardar o envio. Qualquer dúvida estamos à disposição.\n\n— Classul',
    msg_entregue_enabled: '1',
    msg_entregue_template:
      'Olá {nome}! ✅\n\nSeu pedido {pedido} foi enviado/entregue.\n\nAgradecemos muito pela preferência! Se puder, deixe sua avaliação no Google, isso nos ajuda demais:\n🔗 https://g.page/r/CdrHqDugZPp5EBE/review\n\n— Classul',
    google_client_id: process.env.GOOGLE_CLIENT_ID || '',
    google_client_secret: process.env.GOOGLE_CLIENT_SECRET || '',
    google_refresh_token: '',
    google_folder_id: '',
    google_photos_folder_id: '',
    google_inbox_folder_id: '',
    bot_enabled: '0',
    openai_api_key: process.env.OPENAI_API_KEY || '',
    openai_model: 'gpt-4o-mini',
    bot_test_number: '',
    bot_products: DEFAULT_BOT_PRODUCTS,
    bot_system_prompt: DEFAULT_BOT_PROMPT,
    bot_webhook_secret: crypto.randomBytes(16).toString('hex'),
    // 'interno' = bot de IA deste sistema; 'hermes' = o bot da VPS assume; 'off' = ninguém responde.
    bot_engine: 'interno',
    hermes_enabled: '0',
    hermes_token: crypto.randomBytes(24).toString('hex'),
    hermes_webhook_url: ''
  };
}

let pool = null;

// Hook usado apenas pelos testes (pg-mem).
export function _setPoolForTests(testPool) {
  pool = testPool;
}

export function getPool() {
  if (!pool) {
    const connectionString =
      process.env.DATABASE_URL || process.env.POSTGRES_URL || process.env.POSTGRES_PRISMA_URL;
    if (!connectionString) {
      throw new Error(
        'Banco de dados não configurado. Crie um Postgres (Storage → Neon) na Vercel ou defina DATABASE_URL.'
      );
    }
    pool = new pg.Pool({
      connectionString,
      max: 3,
      ssl: /localhost|127\.0\.0\.1/.test(connectionString) ? undefined : { rejectUnauthorized: false }
    });
  }
  return pool;
}

export const q = (text, params) => getPool().query(text, params);

let readyPromise = null;

// Garante o schema uma vez por cold start da função.
export function ensureSchema() {
  if (!readyPromise) {
    readyPromise = initSchema().catch((err) => {
      readyPromise = null;
      throw err;
    });
  }
  return readyPromise;
}

async function initSchema() {
  await q(`
    CREATE TABLE IF NOT EXISTS orders (
      id SERIAL PRIMARY KEY,
      customer_name TEXT NOT NULL,
      phone TEXT,
      description TEXT,
      product_type TEXT,
      case_color TEXT,
      value TEXT,
      due_date TEXT,
      status TEXT NOT NULL DEFAULT 'novo',
      archived INTEGER NOT NULL DEFAULT 0,
      created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
      updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
    )
  `);
  await q(`
    CREATE TABLE IF NOT EXISTS message_log (
      id SERIAL PRIMARY KEY,
      order_id INTEGER NOT NULL,
      status_trigger TEXT NOT NULL,
      phone TEXT,
      body TEXT,
      success INTEGER NOT NULL DEFAULT 0,
      error TEXT,
      created_at TIMESTAMPTZ NOT NULL DEFAULT now()
    )
  `);
  await q(`
    CREATE TABLE IF NOT EXISTS settings (
      key TEXT PRIMARY KEY,
      value TEXT NOT NULL DEFAULT ''
    )
  `);
  await q(`
    CREATE TABLE IF NOT EXISTS clients (
      id SERIAL PRIMARY KEY,
      name TEXT NOT NULL,
      phone TEXT,
      email TEXT,
      company TEXT,
      notes TEXT,
      created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
      updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
    )
  `);
  await q(`
    CREATE TABLE IF NOT EXISTS attachments (
      id SERIAL PRIMARY KEY,
      order_id INTEGER NOT NULL,
      drive_file_id TEXT NOT NULL,
      name TEXT NOT NULL,
      mime_type TEXT,
      size BIGINT,
      web_view_link TEXT,
      created_at TIMESTAMPTZ NOT NULL DEFAULT now()
    )
  `);
  await q(`
    CREATE TABLE IF NOT EXISTS bot_conversations (
      id SERIAL PRIMARY KEY,
      phone TEXT UNIQUE NOT NULL,
      push_name TEXT,
      status TEXT NOT NULL DEFAULT 'active',
      handled_reason TEXT,
      handled_at TIMESTAMPTZ,
      created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
      updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
    )
  `);
  await q(`
    CREATE TABLE IF NOT EXISTS bot_messages (
      id SERIAL PRIMARY KEY,
      phone TEXT NOT NULL,
      role TEXT NOT NULL,
      content TEXT,
      wa_message_id TEXT,
      created_at TIMESTAMPTZ NOT NULL DEFAULT now()
    )
  `);
  await q(`
    CREATE TABLE IF NOT EXISTS employees (
      id SERIAL PRIMARY KEY,
      name TEXT NOT NULL,
      color TEXT,
      active INTEGER NOT NULL DEFAULT 1,
      created_at TIMESTAMPTZ NOT NULL DEFAULT now()
    )
  `);
  await q(`
    CREATE TABLE IF NOT EXISTS order_comments (
      id SERIAL PRIMARY KEY,
      order_id INTEGER NOT NULL,
      author TEXT,
      body TEXT NOT NULL,
      created_at TIMESTAMPTZ NOT NULL DEFAULT now()
    )
  `);
  await q(`
    CREATE TABLE IF NOT EXISTS chat_assignments (
      phone TEXT PRIMARY KEY,
      employee TEXT,
      status TEXT,
      note TEXT,
      updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
    )
  `);
  // Leads do site: cada clique num botão de WhatsApp vira uma linha aqui.
  await q(`
    CREATE TABLE IF NOT EXISTS leads (
      id SERIAL PRIMARY KEY,
      page TEXT,
      label TEXT,
      referrer TEXT,
      user_agent TEXT,
      utm_source TEXT,
      utm_medium TEXT,
      utm_campaign TEXT,
      created_at TIMESTAMPTZ NOT NULL DEFAULT now()
    )
  `);
  await q('CREATE INDEX IF NOT EXISTS idx_leads_created_at ON leads (created_at)');
  // Finanças pessoais (Open Finance via Pluggy): cada banco/cartão conectado é um "item".
  await q(`
    CREATE TABLE IF NOT EXISTS finance_items (
      item_id TEXT PRIMARY KEY,
      connector_name TEXT,
      connector_image TEXT,
      created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
      updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
    )
  `);
  // Área pessoal do Lucas (aba própria, travada por PIN): missões e rotinas.
  await q(`
    CREATE TABLE IF NOT EXISTS lucas_tasks (
      id SERIAL PRIMARY KEY,
      title TEXT NOT NULL,
      notes TEXT,
      priority TEXT NOT NULL DEFAULT 'media',
      status TEXT NOT NULL DEFAULT 'aberta',
      due_date TEXT,
      done_at TIMESTAMPTZ,
      created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
      updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
    )
  `);
  await q(`
    CREATE TABLE IF NOT EXISTS lucas_routines (
      id SERIAL PRIMARY KEY,
      title TEXT NOT NULL,
      time_of_day TEXT,
      days TEXT NOT NULL DEFAULT '0123456',
      active INTEGER NOT NULL DEFAULT 1,
      created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
      updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
    )
  `);
  // Um registro por rotina por dia (dia no fuso America/Sao_Paulo, calculado em JS).
  await q(`
    CREATE TABLE IF NOT EXISTS lucas_routine_logs (
      id SERIAL PRIMARY KEY,
      routine_id INTEGER NOT NULL,
      day TEXT NOT NULL,
      created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
      UNIQUE (routine_id, day)
    )
  `);
  await q('CREATE INDEX IF NOT EXISTS idx_lucas_logs_day ON lucas_routine_logs (day)');
  // Conteúdo servido à extensão (editável pela aba Extensão do sistema).
  await q(`
    CREATE TABLE IF NOT EXISTS quick_messages (
      id SERIAL PRIMARY KEY,
      title TEXT NOT NULL,
      body TEXT NOT NULL,
      sort_order INTEGER NOT NULL DEFAULT 0,
      active INTEGER NOT NULL DEFAULT 1,
      updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
    )
  `);
  await q(`
    CREATE TABLE IF NOT EXISTS photo_sets (
      id SERIAL PRIMARY KEY,
      name TEXT NOT NULL,
      sort_order INTEGER NOT NULL DEFAULT 0,
      created_at TIMESTAMPTZ NOT NULL DEFAULT now()
    )
  `);
  await q(`
    CREATE TABLE IF NOT EXISTS photos (
      id SERIAL PRIMARY KEY,
      set_id INTEGER NOT NULL,
      drive_file_id TEXT NOT NULL,
      name TEXT NOT NULL,
      mime_type TEXT,
      sort_order INTEGER NOT NULL DEFAULT 0,
      created_at TIMESTAMPTZ NOT NULL DEFAULT now()
    )
  `);
  await q(`
    CREATE TABLE IF NOT EXISTS catalog_products (
      id SERIAL PRIMARY KEY,
      name TEXT NOT NULL,
      short_label TEXT,
      has_size INTEGER NOT NULL DEFAULT 0,
      is_case INTEGER NOT NULL DEFAULT 0,
      sort_order INTEGER NOT NULL DEFAULT 0,
      active INTEGER NOT NULL DEFAULT 1
    )
  `);

  // Conversas acompanhadas: só as marcadas por você são observadas e analisadas.
  await q(`
    CREATE TABLE IF NOT EXISTS watched_chats (
      phone TEXT PRIMARY KEY,
      chat_name TEXT,
      added_by TEXT,
      last_message_at TIMESTAMPTZ,
      created_at TIMESTAMPTZ NOT NULL DEFAULT now()
    )
  `);
  // Histórico bruto da conversa acompanhada (o bot não responde nada aqui).
  await q(`
    CREATE TABLE IF NOT EXISTS chat_messages (
      id SERIAL PRIMARY KEY,
      phone TEXT NOT NULL,
      wa_message_id TEXT,
      from_me INTEGER NOT NULL DEFAULT 0,
      push_name TEXT,
      body TEXT,
      media_type TEXT,
      created_at TIMESTAMPTZ NOT NULL DEFAULT now()
    )
  `);
  // Arquivo recebido no WhatsApp que ainda não tem pedido para morar.
  await q(`
    CREATE TABLE IF NOT EXISTS chat_files (
      id SERIAL PRIMARY KEY,
      phone TEXT NOT NULL,
      chat_name TEXT,
      drive_file_id TEXT NOT NULL,
      name TEXT NOT NULL,
      mime_type TEXT,
      size BIGINT,
      web_view_link TEXT,
      attached_order_id INTEGER,
      created_at TIMESTAMPTZ NOT NULL DEFAULT now()
    )
  `);

  // Sugestões que o analista gera lendo a conversa acompanhada.
  // Nada entra no quadro sozinho: tudo espera sua confirmação.
  await q(`
    CREATE TABLE IF NOT EXISTS order_suggestions (
      id SERIAL PRIMARY KEY,
      phone TEXT NOT NULL,
      chat_name TEXT,
      order_id INTEGER,
      kind TEXT NOT NULL DEFAULT 'novo_pedido',
      stage TEXT,
      status TEXT NOT NULL DEFAULT 'pendente',
      summary TEXT,
      data TEXT,
      created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
      resolved_at TIMESTAMPTZ,
      resolved_by TEXT
    )
  `);

  // Registro das duas mãos da conversa com o Hermes: o que ele pediu ao sistema
  // e o que o sistema avisou para ele.
  await q(`
    CREATE TABLE IF NOT EXISTS hermes_events (
      id SERIAL PRIMARY KEY,
      direction TEXT NOT NULL,
      name TEXT NOT NULL,
      args TEXT,
      result TEXT,
      ok INTEGER NOT NULL DEFAULT 1,
      error TEXT,
      created_at TIMESTAMPTZ NOT NULL DEFAULT now()
    )
  `);

  for (const ddl of [
    'ALTER TABLE orders ADD COLUMN IF NOT EXISTS client_id INTEGER',
    'ALTER TABLE orders ADD COLUMN IF NOT EXISTS drive_folder_id TEXT',
    'ALTER TABLE orders ADD COLUMN IF NOT EXISTS delivered_at TIMESTAMPTZ',
    "ALTER TABLE orders ADD COLUMN IF NOT EXISTS payment_status TEXT NOT NULL DEFAULT 'pendente'",
    'ALTER TABLE orders ADD COLUMN IF NOT EXISTS pickup_code TEXT',
    'ALTER TABLE orders ADD COLUMN IF NOT EXISTS pickup_time TEXT',
    // Venda de estojo avulso (sem placa) e o tamanho de placa que o estojo comporta.
    'ALTER TABLE orders ADD COLUMN IF NOT EXISTS case_only INTEGER NOT NULL DEFAULT 0',
    'ALTER TABLE orders ADD COLUMN IF NOT EXISTS case_size TEXT',
    // Tamanho vale para qualquer pedido: da placa, ou da placa que cabe no estojo avulso.
    'ALTER TABLE orders ADD COLUMN IF NOT EXISTS size TEXT',
    'UPDATE orders SET size = case_size WHERE size IS NULL AND case_size IS NOT NULL',
    // Produto vendido (catálogo do site). O antigo case_only vira o produto "Estojo avulso".
    'ALTER TABLE orders ADD COLUMN IF NOT EXISTS product TEXT',
    "UPDATE orders SET product = 'Estojo avulso' WHERE product IS NULL AND case_only = 1",
    'ALTER TABLE orders ADD COLUMN IF NOT EXISTS created_by TEXT',
    'ALTER TABLE orders ADD COLUMN IF NOT EXISTS updated_by TEXT',
    'ALTER TABLE chat_assignments ADD COLUMN IF NOT EXISTS chat_name TEXT',
    "ALTER TABLE attachments ADD COLUMN IF NOT EXISTS category TEXT NOT NULL DEFAULT 'arquivo'",
    'ALTER TABLE finance_items ADD COLUMN IF NOT EXISTS label TEXT',
    'ALTER TABLE leads ADD COLUMN IF NOT EXISTS utm_source TEXT',
    'ALTER TABLE leads ADD COLUMN IF NOT EXISTS utm_medium TEXT',
    'ALTER TABLE leads ADD COLUMN IF NOT EXISTS utm_campaign TEXT',
    'ALTER TABLE watched_chats ADD COLUMN IF NOT EXISTS last_analysis_at TIMESTAMPTZ'
  ]) {
    try {
      await q(ddl);
    } catch {
      try {
        await q(ddl.replace(' IF NOT EXISTS', ''));
      } catch {
        // coluna já existe
      }
    }
  }
  for (const [key, value] of Object.entries(defaultSettings())) {
    await q('INSERT INTO settings (key, value) VALUES ($1, $2) ON CONFLICT (key) DO NOTHING', [key, value]);
  }

  // Semeia o conteúdo da extensão só se as tabelas estiverem vazias — depois
  // disso, quem manda é o que o usuário editou no sistema.
  const { rows: qmCount } = await q('SELECT COUNT(*)::int AS n FROM quick_messages');
  if (!qmCount[0].n) {
    for (const [i, m] of DEFAULT_QUICK_MESSAGES.entries()) {
      await q('INSERT INTO quick_messages (title, body, sort_order) VALUES ($1, $2, $3)', [m.title, m.text, i]);
    }
  }
  const { rows: catCount } = await q('SELECT COUNT(*)::int AS n FROM catalog_products');
  if (!catCount[0].n) {
    for (const [i, p] of DEFAULT_CATALOG.entries()) {
      await q(
        'INSERT INTO catalog_products (name, short_label, has_size, is_case, sort_order) VALUES ($1, $2, $3, $4, $5)',
        [p.name, p.short_label, p.has_size, p.is_case, i]
      );
    }
  }
}

export async function getSettings() {
  const { rows } = await q('SELECT key, value FROM settings');
  return Object.fromEntries(rows.map((r) => [r.key, r.value]));
}

export async function setSettings(partial) {
  for (const [key, value] of Object.entries(partial)) {
    if (!DEFAULT_SETTINGS_KEYS.includes(key)) continue;
    await q(
      'INSERT INTO settings (key, value) VALUES ($1, $2) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value',
      [key, String(value ?? '')]
    );
  }
  return getSettings();
}
